    ...
    <SafeAreaView className="flex-1 bg-background">
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        <View className="mb-6 flex-row justify-between items-end border-b-2 border-divider pb-2">
          <View>
            <MachineText variant="label" className="text-accent mb-1">SYSTEM://OVERVIEW</MachineText>
        ...
        {suggestions.length > 0 && (
          <View className="mb-8">
            <MachineText variant="label" className="mb-2 text-accent">INPUT_SIGNALS ({suggestions.length})</MachineText>
            <View className="gap-3">
              {suggestions.slice(0, 2).map((suggestion: SuggestionItem) => (
                <HardCard key={suggestion._id} variant="default" className="border-accent" label={`SIG-${suggestion.type}`}>
                  <View className="flex-row items-start justify-between">
                    ...
                    <View className="gap-2">
                      {suggestion.type === "PLAN_RESET" && (
                        ...
                        >
                          <MachineText className="text-accent-foreground text-xs font-bold">EXECUTE</MachineText>
                        </Button>
                      )}
                      {suggestion.type === "GENTLE_RETURN" && (
                        ...
                        >
                          <MachineText className="text-accent-foreground text-xs font-bold">RESUME</MachineText>
                        </Button>
                      )}
                      {suggestion.type === "MICRO_RECOVERY_PROTOCOL" && (
                        <View className="gap-2">
                          ...
                          >
                            <MachineText className="text-accent-foreground text-xs font-bold">DO TINY WIN</MachineText>
                          </Button>
                          <Button
                            size="sm"
                          ...
                          >
                            <MachineText className="text-xs font-bold text-foreground">REFLECT</MachineText>
                          </Button>
                        </View>
                      )}
                    </View>
          ...
          <HardCard label="CMD_LINE_INPUT" className="bg-surface">
            <View className="gap-3">
              ...
              <View className="flex-row gap-3">
                ...
                >
                  {isCreating ? <Spinner size="sm" color="white" /> : <MachineText className="text-background font-bold">ENTER</MachineText>}
                </Button>
              </View>
            </View>
          </HardCard>
